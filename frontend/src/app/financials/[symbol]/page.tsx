...
function BigChart({
    ...
    return (
        <section
            className="rounded-[28px] border border-white/8 bg-[linear-gradient(180deg,rgba(12,19,29,0.96),rgba(8,12,18,0.96))] p-5">
          ...
          Backend data
            ...
            {points.length > 0 ? (
                <div className="grid min-h-[320px] grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
                    {points.map((point) => (
                        <div key={point.label} className="flex flex-col justify-end gap-3">
                            <div
                                className="flex min-h-[240px] items-end rounded-[24px] border border-white/8 bg-[#121821] p-3">
                                <div
                ...
                <div
                    className="flex min-h-[320px] items-center justify-center rounded-[24px] border border-dashed border-white/8 bg-[#121821] text-sm text-[#6F8197]">
                    Data is missing.
                </div>
            )}
        </section>
...
export default async function FinancialsPage({params}: FinancialsPageProps) {
    ...
    return (
        <div className="relative min-h-screen bg-background text-foreground">
            <div
                className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(61,217,179,0.08),transparent_38%),linear-gradient(180deg,rgba(10,16,24,0.12),rgba(5,9,14,0.18))]"/>
            <div className="terminal-grid pointer-events-none absolute inset-0 opacity-40"/>

            <MarketlyNavbar currentSymbol={ticker}/>

            <main
                className="relative mx-auto flex min-h-screen w-full max-w-[1440px] flex-col px-4 pb-16 pt-[92px] sm:px-6 lg:px-10">
                <section className="mx-auto w-full max-w-[1180px] space-y-6">
                    <div
                        className="overflow-hidden rounded-[34px] border border-white/8 bg-[linear-gradient(145deg,rgba(13,22,33,0.98),rgba(8,12,18,0.96))]">
                        <div
                            className="flex flex-wrap items-start justify-between gap-5 border-b border-white/8 px-6 py-6 sm:px-8">
                            <div>
                                <div
                                    className="inline-flex items-center gap-2 rounded-full border border-[#3DD9B3]/18 bg-[#3DD9B3]/10 px-3 py-1 text-[10px] uppercase tracking-[0.28em] text-[#9CEBD8]">
                                ...
                                <p className="mt-3 max-w-[760px] text-sm leading-7 text-[#8EA0B8]">
                                    Full backend view for {ticker}, with reported history, quote context, analyst
                                    distribution, and statement coverage.
                                </p>
                            </div>
                    ...
                    {data ? (
                        <>
                            <div className="grid gap-4 md:grid-cols-4">
                                {[
                                    ...
                                    <div key={item.label}
                                         className="rounded-[24px] border border-white/8 bg-[#0B1118] p-5">
                                        <p className="text-[11px] uppercase tracking-[0.24em] text-[#9FB3C8]">{item.label}</p>
                        ...
                        <section
                            className="rounded-[30px] border border-dashed border-white/10 bg-[#0B1118] p-8 text-center">
                            <div className="mx-auto max-w-[680px]">
                                <div
                                    className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-[#F3C969]/20 bg-[#F3C969]/10">
                                ...
                                <p className="mt-4 text-sm leading-7 text-[#8EA0B8]">
                                    The backend did not return a usable financial payload for {ticker}. Try re-running
                                    the analysis workspace or switching to a different symbol while the provider
                                    recovers.
                                </p>
