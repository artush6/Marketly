...
-- ---------------------------------------------------------------------------
-- Operational cache / provider snapshots used by the current backend
-- ---------------------------------------------------------------------------
...
create index if not exists market_data_cache_expires_at_idx on public.market_data_cache (expires_at);

drop trigger if exists market_data_cache_set_updated_at on public.market_data_cache;
create trigger market_data_cache_set_updated_at
...
create index if not exists market_data_snapshots_lookup_idx
...
on public.financial_statement_rows (symbol, statement_type, period_end desc);
...
create index if not exists financial_metrics_symbol_metric_idx
